...
public class ArbolBinarioArtistas {
    ...
    private NodoArtista insertar(NodoArtista nodo, Artista artista) {
        if (nodo == null) {
            ...
            // Si el nombre del artista es menor que el nombre del artista del nodo, se inserta en el subárbol izquierdo
            nodo.izquierdo = insertar(nodo.izquierdo, artista);
        } else if (artista.getNombre().compareTo(nodo.artista.getNombre()) > 0) {
            // Si el nombre del artista es mayor que el nombre del artista del nodo, se inserta en el subárbol derecho
            nodo.derecho = insertar(nodo.derecho, artista);
        }
        return nodo;
    }

    // Busca un artista en el árbol a partir del nombre y devuelve el artista si se encuentra, null si no
    public Artista buscar(String nombre) {
        return buscar(raiz, nombre);
    }

    // Busca un artista en el árbol a partir de un nodo dado y del nombre, y devuelve el artista si se encuentra, null si no
    private Artista buscar(NodoArtista nodo, String nombre) {
        if (nodo == null) {
            ...
            // Si el nombre del artista es menor que el nombre del artista del nodo, se busca en el subárbol izquierdo
            return buscar(nodo.izquierdo, nombre);
        } else if (nombre.compareTo(nodo.artista.getNombre()) > 0) {
            // Si el nombre del artista es mayor que el nombre del artista del nodo, se busca en el subárbol derecho
            return buscar(nodo.derecho, nombre);
        } else {
            // Si el nombre del artista es igual al nombre del artista del nodo, se encontró el artista en el árbol
            return nodo.artista;
        }
    }
    ...
    private NodoArtista eliminar(NodoArtista nodo, String nombre) {
        if (nodo == null) {
            ...
            // Si el nombre del artista es igual al nombre del artista del nodo, se encontró el nodo que contiene el artista
            if (nodo.izquierdo == null) {
                ...
                // Si el nodo tiene ambos subárboles, se busca el sucesor en orden del nodo y se reemplaza el artista
